...
export default function EditProductPage({ params }: EditProductPageProps) {
  ...
  const loadProduct = useCallback(async (id: string) => {
    ...
    try {
      const product = await FirebaseProductsService.getProduct(id)
      if (!product) {
      ...
      setFormData({
        ...
        mrp: product.originalPrice?.toString() || product.price.toString(),
        stock: product.stockQuantity.toString(),
        sku: "", // Firebase products don't have SKU
        weight: "", // Firebase products don't have weight
        dimensions: "", // Firebase products don't have dimensions
        material: "", // Firebase products don't have material
        care: "", // Firebase products don't have care
        status: product.inStock ? "active" : "inactive",
      })

      setImages(product.images)
      setFeatures(product.tags && product.tags.length > 0 ? product.tags : [""])

      const specs = product.specifications ? Object.entries(product.specifications) : []
      setSpecifications(specs.length > 0 ? specs.map(([key, value]) => ({ key, value: String(value) })) : [{ key: "", value: "" }])
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update product
      await FirebaseProductsService.updateProduct(productId, updateData)

      alert(`Product "${updateData.name}" has been updated successfully!`)
      router.push("/admin/products")
    } catch (error) {
